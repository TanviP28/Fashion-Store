...
autocomplete(document.getElementById("myInput"), countries);
    ...
    <div id = "profile">
		...
		try{
			...
			ResultSet rs	= stmt.executeQuery("SELECT * FROM login_details where user_id="+userid +";");
			while (rs.next())
			{
				%>
							
            <p> <b>ACCOUNT ID	:</b> 	<%= rs.getInt(1)%></p>
            <p> <b>NAME :</b>	<%= rs.getString(2)%> </p>
            <P> <b>EMAIL ADDRESS :</b> 	<%= rs.getString(3)%> </P>
            <P> <b>PHONE NUMBER :</b> 	<%= rs.getString(4)%> </P>          
		...
		%>          
            <div class="text-center"> 
				<button class = "btn btn-dark" style="margin-bottom: 10px;width: 50%;" onClick="location.href='../jsp/viewhistory.jsp'"> VIEW ORDER HISTORY </button> 
                <br><button class = "btn btn-dark" type = "button" onclick="location.href = '../jsp/logout.jsp';"> LOGOUT </button>
            </div>
        </div>
    </div>
